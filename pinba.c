...
#define PINBA_USEC_PER_SEC 1000000   /* microseconds in one second */
#define PINBA_HASHED_TAGS_CHUNK 4096 /* initial size / growth step of the hashed-tags buffer */
#define PINBA_HOSTNAME_BUF_SIZE 256  /* scratch buffer for the request hostname */

#define timeval_cvt(a, b)        \
  do {                           \
    (a)->tv_sec = (b)->tv_sec;   \
    (a)->tv_usec = (b)->tv_usec; \
  } while (0);
...
static inline int php_pinba_timer_stop(pinba_timer_t *t, struct timeval *pnow,
                                       struct rusage *pu) /* {{{ */
{
  struct timeval now;
  struct rusage u;
...
static int php_pinba_tags_to_hashed_string(pinba_timer_tag_t **tags, int tags_num,
                                           char **hashed_tags, size_t *hashed_tags_len) /* {{{ */
{
  int i;
  char *buf;
  size_t buf_len;
  ...
  *hashed_tags_len = 0;
  ...
  buf_len = PINBA_HASHED_TAGS_CHUNK;
  wrote_len = 0;
  ...
  for (i = 0; i < tags_num; i++) {
    if (buf_len <= (wrote_len + tags[i]->name_len + 2 + tags[i]->value_len + 1)) {
      buf_len =
          wrote_len + tags[i]->name_len + 2 + tags[i]->value_len + 1 + PINBA_HASHED_TAGS_CHUNK;
...
static pinba_sockaddr *php_pinba_connect_socket(const char *host, const char *port) /* {{{ */
{
  pinba_sockaddr *sa;
  ...
  hostport_len = spprintf(&hostport, 0, "%s:%s", host, port);

  sa = zend_hash_str_find_ptr(&resolver_cache, hostport, hostport_len);
  if (sa) {
    if ((now - sa->sockaddr_time) < PINBA_G(resolve_interval)) {
      efree(hostport);
      return sa;
    }
    /* delete the value and proceed to re-resolve */
    zend_hash_str_del(&resolver_cache, hostport, hostport_len);
    sa = NULL;
  }
  ...
  if (status != 0) {
    efree(hostport);
    php_error_docref(NULL, E_WARNING, "failed to resolve Pinba server hostname '%s': %s", host,
                     gai_strerror(status));
    return NULL;
  }
  ...
  for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
    ...
    if (fd >= 0) {
      sa = pecalloc(1, sizeof(pinba_sockaddr), 1);
      memcpy(&sa->sockaddr, ai_ptr->ai_addr, ai_ptr->ai_addrlen);
      sa->sockaddr_len = ai_ptr->ai_addrlen;
      sa->sockaddr_time = now;
      sa->fd = fd;

      zend_hash_str_update_ptr(&resolver_cache, hostport, hostport_len, sa);
      break;
    }
  }
...
static int php_pinba_init_socket(pinba_collector *collectors, int n_collectors) /* {{{ */
{
  ...
  for (i = 0; i < n_collectors; i++) {
    ...
    pinba_sockaddr *sa = php_pinba_connect_socket(collector->host, collector->port);
    if (!sa) {
...
static inline char *_pinba_fetch_global_var(char *name, int name_len) /* {{{ */
{
  ...
  if (!PINBA_G(in_rshutdown) && (Z_TYPE(PG(http_globals)[TRACK_VARS_SERVER]) == IS_ARRAY ||
...
static inline Pinba__Request *php_create_pinba_packet(pinba_client_t *client,
                                                      const char *custom_script_name,
                                                      int flags) /* {{{ */
{
  HashTable dict;
  HashTable *tags;
  ...
  char hostname[PINBA_HOSTNAME_BUF_SIZE];
  char *tag_value;
  pinba_req_data *req_data = &PINBA_G(tmp_req_data);
  int timers_num;
  int tags_cnt;
  int *tag_ids = NULL;
  int *tag_value_ids = NULL;
  int i;
  int n;
  ...
  if (client) {
    ...
    struct timeval ru_utime = {0, 0};
    struct timeval ru_stime = {0, 0};
    struct rusage u;
  ...
  if (timers_num > 0) {
    ...
    for (zend_hash_internal_pointer_reset_ex(timers, &pos);
      ...
      if (php_pinba_tags_to_hashed_string(t->tags, t->tags_num, &hashed_tags, &hashed_tags_len) !=
          SUCCESS) {
        continue;
      }
  ...
  if (timers_num > 0) {
    ...
    if (!request->timer_hit_count || !request->timer_tag_count || !request->timer_value ||
        !request->timer_ru_stime || !request->timer_ru_utime) {
      pinba__request__free_unpacked(request, NULL);
      return NULL;
    }
...
#define PINBA_FREE_BUFFER() PROTOBUF_C_BUFFER_SIMPLE_CLEAR(&_buf)

static inline int php_pinba_req_data_send(pinba_client_t *client, const char *custom_script_name,
                                          int flags) /* {{{ */
{
  int ret = SUCCESS;
  Pinba__Request *request;
...
static void php_pinba_flush_data(const char *custom_script_name, long flags) /* {{{ */
{
  ...
  zend_hash_apply_with_arguments(&EG(regular_list), (apply_func_args_t)php_pinba_timer_stop_helper,
                                 3, (void *)flags, (void *)&now, (void *)&u);
  ...
  *tags = (pinba_timer_tag_t **)ecalloc(num, sizeof(pinba_timer_tag_t *));
  ZEND_HASH_FOREACH_STR_KEY_VAL_IND(array, tag_name_str, value) {
    ...
    if (tag_name_str) {
      ...
      php_error_docref(
          NULL, E_WARNING,
          "tags can only have string names (i.e. tags array cannot contain numeric indexes)");
      php_pinba_timer_tags_dtor(*tags, i);
      efree(*tags);
      return FAILURE;
    }
...
{
  pinba_client_t *client = php_pinba_client_object(object);

  if (!client->data_sent && (client->flags & PINBA_AUTO_FLUSH) != 0) {
    if (client->collectors_initialized ||
        php_pinba_init_socket(client->collectors, client->n_collectors) != FAILURE) {
...
static PHP_FUNCTION(pinba_timer_start) {
  ...
  ZEND_PARSE_PARAMETERS_START(1, 3)  // NOLINT(readability-isolate-declaration): Zend macro
  ...
  if (hit_count <= 0) {
    php_error_docref(NULL, E_WARNING, "hit_count must be greater than 0 (%ld was passed)",
                     hit_count);
    RETURN_FALSE;
  }
...
static PHP_FUNCTION(pinba_timer_add) {
  ...
  ZEND_PARSE_PARAMETERS_START(2, 4)  // NOLINT(readability-isolate-declaration): Zend macro
  Z_PARAM_ARRAY_EX(tags_array, 0, 1)
  ...
  t->started = 0;
  t->hit_count = hit_count;
  time_l = (unsigned long)(value * (double)PINBA_USEC_PER_SEC);
  t->value.tv_sec = time_l / PINBA_USEC_PER_SEC;
  t->value.tv_usec = time_l % PINBA_USEC_PER_SEC;
...
static PHP_FUNCTION(pinba_timer_tags_merge) {
  ...
  ZEND_PARSE_PARAMETERS_START(2, 2)  // NOLINT(readability-isolate-declaration): Zend macro
  ...
  for (i = 0; i < tags_num; i++) {
    ...
    for (j = 0; j < t->tags_num; j++) {
      if (t->tags[j]->name_len == new_tags[i]->name_len &&
          memcmp(t->tags[j]->name, new_tags[i]->name, new_tags[i]->name_len) == 0) {
        found = 1;
        break;
      }
    }
    if (found == 1) {
      ...
      t->tags =
          (pinba_timer_tag_t **)erealloc(t->tags, (t->tags_num + 1) * sizeof(pinba_timer_tag_t *));
...
static PHP_FUNCTION(pinba_flush) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "|sl", &script_name, &script_name_len, &flags) !=
      SUCCESS) {
    return;
  }
...
/* {{{ proto void pinba_reset()
   Reset the data */
static PHP_FUNCTION(pinba_reset) { php_pinba_reset_data(); }
/* }}} */
...
static PHP_METHOD(PinbaClient, __construct) {
  ...
  ZEND_PARSE_PARAMETERS_START(1, 2)  // NOLINT(readability-isolate-declaration): Zend macro
...
static PHP_METHOD(PinbaClient, setRusage) {
  ...
  ZEND_PARSE_PARAMETERS_START(1, 1)  // NOLINT(readability-isolate-declaration): Zend macro
...
static void php_pinba_client_timer_add_set(INTERNAL_FUNCTION_PARAMETERS, int add) /* {{{ */
{
  ...
  if (php_pinba_tags_to_hashed_string(new_tags, tags_num, &hashed_tags, &hashed_tags_len) !=
      SUCCESS) {
    php_pinba_timer_tags_dtor(new_tags, tags_num);
    efree(new_tags);
    RETURN_FALSE;
...
zend_function_entry pinba_functions[] = {
    PINBA_FUNC(pinba_timer_start) PINBA_FUNC(pinba_timer_add) PINBA_FUNC(pinba_timer_stop)
        PINBA_FUNC(pinba_timer_delete) PINBA_FUNC(pinba_timer_data_merge)
            PINBA_FUNC(pinba_timer_data_replace) PINBA_FUNC(pinba_timer_tags_merge)
                PINBA_FUNC(pinba_timer_tags_replace) PINBA_FUNC(pinba_flush) PINBA_FUNC(pinba_reset)
                    PINBA_FUNC(pinba_get_info) PINBA_FUNC(pinba_get_data)
                        PINBA_FUNC(pinba_timer_get_info) PINBA_FUNC(pinba_timers_stop)
                            PINBA_FUNC(pinba_timers_get) PINBA_FUNC(pinba_script_name_set)
                                PINBA_FUNC(pinba_hostname_set) PINBA_FUNC(pinba_server_name_set)
                                    PINBA_FUNC(pinba_schema_set) PINBA_FUNC(pinba_request_time_set)
                                        PINBA_FUNC(pinba_tag_set) PINBA_FUNC(pinba_tag_get)
                                            PINBA_FUNC(pinba_tag_delete) PINBA_FUNC(pinba_tags_get)
                                                PHP_FE_END};
/* }}} */
...
zend_function_entry pinba_client_methods[] = {
    PHP_ME(PinbaClient, __construct, arginfo___construct, ZEND_ACC_PUBLIC) PHP_ME(
        PinbaClient, setHostname, arginfo_setvalue, ZEND_ACC_PUBLIC)
        PHP_ME(PinbaClient, setScriptname, arginfo_setvalue, ZEND_ACC_PUBLIC) PHP_ME(
            PinbaClient, setServername, arginfo_setvalue, ZEND_ACC_PUBLIC)
            PHP_ME(PinbaClient, setRequestCount, arginfo_setvalue, ZEND_ACC_PUBLIC) PHP_ME(
                PinbaClient, setDocumentSize, arginfo_setvalue, ZEND_ACC_PUBLIC)
                PHP_ME(PinbaClient, setMemoryPeak, arginfo_setvalue, ZEND_ACC_PUBLIC) PHP_ME(
                    PinbaClient, setMemoryFootprint, arginfo_setvalue, ZEND_ACC_PUBLIC)
                    PHP_ME(PinbaClient, setRusage, arginfo_setvalue, ZEND_ACC_PUBLIC) PHP_ME(
                        PinbaClient, setRequestTime, arginfo_setvalue, ZEND_ACC_PUBLIC)
                        PHP_ME(PinbaClient, setStatus, arginfo_setvalue, ZEND_ACC_PUBLIC)
                            PHP_ME(PinbaClient, setSchema, arginfo_setvalue, ZEND_ACC_PUBLIC)
                                PHP_ME(PinbaClient, setTag, arginfo_settag, ZEND_ACC_PUBLIC) PHP_ME(
                                    PinbaClient, setTimer, arginfo_settimer, ZEND_ACC_PUBLIC)
                                    PHP_ME(PinbaClient, addTimer, arginfo_settimer, ZEND_ACC_PUBLIC)
                                        PHP_ME(PinbaClient, send, arginfo_send, ZEND_ACC_PUBLIC)
                                            PHP_ME(PinbaClient, getData, arginfo_getdata,
                                                   ZEND_ACC_PUBLIC) PHP_FE_END};
/* }}} */

static void php_pinba_sa_dtor(zval *zv) /* {{{ */
{
  pinba_sockaddr *sa = Z_PTR_P(zv);
  if (sa->fd >= 0) {
    close(sa->fd);
  }
  free(sa);
...
static PHP_INI_MH(OnUpdateCollectorAddress) /* {{{ */
{
  ...
  for (tmp = copy; (address = strsep(&tmp, ", ")) && (address[0] != '\0'); /**/) {
    ...
    new_collector->port =
        (new_service == NULL) ? strdup(PINBA_COLLECTOR_DEFAULT_PORT) : strdup(new_service);
  }
 ...
 */
PHP_INI_BEGIN()
STD_PHP_INI_ENTRY("pinba.server", NULL, PHP_INI_ALL, OnUpdateCollectorAddress, collector_address,
                  zend_pinba_globals, pinba_globals)
STD_PHP_INI_ENTRY("pinba.resolve_interval", "60", PHP_INI_ALL, OnUpdateLongGEZero, resolve_interval,
                  zend_pinba_globals, pinba_globals)
STD_PHP_INI_ENTRY("pinba.enabled", "0", PHP_INI_ALL, OnUpdateBool, enabled, zend_pinba_globals,
                  pinba_globals)
STD_PHP_INI_ENTRY("pinba.auto_flush", "1", PHP_INI_ALL, OnUpdateBool, auto_flush,
                  zend_pinba_globals, pinba_globals)
PHP_INI_END()
/* }}} */
...
static PHP_MINIT_FUNCTION(pinba) {
  ...
  le_pinba_timer = zend_register_list_destructors_ex(php_timer_resource_dtor, NULL, "pinba timer",
                                                     module_number);

  REGISTER_LONG_CONSTANT("PINBA_FLUSH_ONLY_STOPPED_TIMERS", PINBA_FLUSH_ONLY_STOPPED_TIMERS,
                         CONST_CS | CONST_PERSISTENT);
  REGISTER_LONG_CONSTANT("PINBA_FLUSH_RESET_DATA", PINBA_FLUSH_RESET_DATA,
                         CONST_CS | CONST_PERSISTENT);
  REGISTER_LONG_CONSTANT("PINBA_ONLY_STOPPED_TIMERS", PINBA_FLUSH_ONLY_STOPPED_TIMERS,
                         CONST_CS | CONST_PERSISTENT);
  REGISTER_LONG_CONSTANT("PINBA_ONLY_RUNNING_TIMERS", PINBA_ONLY_RUNNING_TIMERS,
                         CONST_CS | CONST_PERSISTENT);
...
static PHP_MSHUTDOWN_FUNCTION(pinba) {
  ...
  zend_hash_destroy(&resolver_cache);
  return SUCCESS;
}
/* }}} */
...
static PHP_RINIT_FUNCTION(pinba) {
  ...
  if (zend_is_auto_global_str("_SERVER", sizeof("_SERVER") - 1)) {
    tmp = zend_hash_str_find(HASH_OF(&PG(http_globals)[TRACK_VARS_SERVER]), "SCRIPT_NAME",
                             sizeof("SCRIPT_NAME") - 1);
    if (tmp && Z_TYPE_P(tmp) == IS_STRING && Z_STRLEN_P(tmp) > 0) {
      PINBA_G(script_name) = estrndup(Z_STRVAL_P(tmp), Z_STRLEN_P(tmp));
    }

    tmp = zend_hash_str_find(HASH_OF(&PG(http_globals)[TRACK_VARS_SERVER]), "SERVER_NAME",
...
/* {{{ pinba_module_entry
 */
zend_module_entry pinba_module_entry = {STANDARD_MODULE_HEADER, "pinba",
                                        pinba_functions,        PHP_MINIT(pinba),
                                        PHP_MSHUTDOWN(pinba),   PHP_RINIT(pinba),
                                        PHP_RSHUTDOWN(pinba),   PHP_MINFO(pinba),
                                        PHP_PINBA_VERSION,      STANDARD_MODULE_PROPERTIES};
/* }}} */
